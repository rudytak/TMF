...
Velikost magentických momentů bude záviset na teplotě, velikosti a materiálových vlastnostech magnetů (např. jejich chemickém složení a kvalitě), ale to, jaká je pravá velikost magnetických momentů $|\vec{m}_0|$, je momentálně nepodstatné a určíme ji později (viz \autoref{sec:remanence_measurement}).
...
Další důležitou datovou strukturou je něco, co nazveme $\Omega$-stav. Tato datová struktura má za úkol uložit celkový stav systému (tedy stavy všech $m$ spinnerů) a provádět na nich operace potřebné k řešení diferenciálních rovnic pomocí RK metod. Zároveň $\Omega$-stav disponuje funkcí pro výpočet všech momentů sil pro všechny spinnery.

Hlavní vlastností $\Omega$-stavů je tedy pole spinnerů $\mathbb{P} = \{ \mathbb{S}_1, \mathbb{S}_2, \ldotp ,\mathbb{S}_m \}$ a na libovolný prvek tohoto pole se budeme odkazovat následovně: $\Omega_{}^{\mathbb{P}}(m) = \mathbb{S}_m$.

Jelikož úkolem $\Omega$-stavu je i určení všech momentů sil, musí mít i vlastnost značenou $A$, což je pole dopočítaných úhlových zrychlení jednotlivých spinnerů: $A = \{ \alpha_1, \alpha_2, \ldotp, \alpha_m \} $ (na jednotlivé prvky se opět odkazujeme takto: $\Omega_{}^{A}(m) = \alpha_m$). Nyní je na řadě provést formalizaci řídících rovnic \ref{eq:sim_equations} a \ref{eq:sim_equations2}:
\begin{equation}
    ...
    \begin{gathered}
        ...
        \times (\mathbb{S}^{P}(j) -  \mathbb{S}^{S})\\
        \tau_{mag}(\mathbb{S}, m_{external}, P_{external}) =
        \sum_{j=1}^{\mathbb{S}^{n}}
        \mathbb{S}^{m}(j) \times B(\mathbb{S}^{P}(j)-P_{external},m_{external})\\
    \end{gathered}
\end{equation}

Nyní, když jsme formalizovali silové a magnetické interakce z jednoho externího magentického dipólu, stačí určit celkový moment síly $\tau_{tot}$, což je součet obou složek ze všech magnetů všech ostatních spinnerů, a z toho jednoduše dopočítat úhlové zrychlení:
\begin{equation}
...
Poslední vlastností $\Omega$-stavu je krátky časový úsek $\Delta t$. Z výpočtu zrychlení je pak
provedena predikce budoucího stavu $\Omega'$, který by měl nastat po uplynutí $\Delta t$:

\begin{equation}
    \label{eq:sim_equations_formalized3}
    \begin{gathered}
        {\Omega'}^{\mathbb{P}}(k)^{\omega} = \mathbb{S}_k^{\omega} + \Omega^{\Delta t} \cdot \alpha_k
        \quad
        \forall k \in {1, \ldots, \Omega^{m}} \\
    \end{gathered}
\end{equation}
...
Poslední datovou strukturou jsou tzv. $\Phi$-stavy, které jsou velmi podobné $\Omega$-stavům svou funkcí. Jejich účelem je opět umožnit použití RK metod - v průběhu jednoho simulačního kroku tedy dochází k dvojitému numerickému integrování. Nejdříve, abychom získali z úhlového zrychlení úhlovou rychlost, a poté poduhé, abychom z úhlové rychlosti získali úhel
($\alpha_k \xRightarrow{RK} \mathbb{S}_k^{\omega}  \xRightarrow{RK} \mathbb{S}_k^{\varphi}$).

Vlastnosti $\Phi$-stavů jsou stejné jako u $\Omega$-stavů, kromě pole $A$, které je nahrazeno polem $O$\footnote{$\Phi{}^{O} = \{ \omega_1, \omega_2, \ldots, \omega_m\}; \Phi{}^{O}(m) = \omega_m$}, které místo úhlových zrychlení obsahuje úhlové rychlosti každého spinneru. $\Omega$-stavy a $\Phi$-stavy na sebe navazují následovně:
...
\begin{equation}
    ...
    \begin{gathered}
        {\Phi'}^{\mathbb{P}}(k)^{\omega} = \mathbb{S}_k^{\varphi} + \Phi^{\Delta t} \cdot \omega_k
        \quad
        \forall k \in {1, \ldots, \Phi^{m}} \\
    \end{gathered}
\end{equation}
...
Jak jsme již avizovali, pomocí $\Omega$-stavů a $\Phi$-stavů provedeme v každém iteračním kroku 2 numerické integrace: $\alpha_k \xRightarrow{RK} \mathbb{S}_k^{\omega}  \xRightarrow{RK} \mathbb{S}_k^{\varphi}$. K simulování jsme používali metodu RK4, protože je nejvíce známá a při porovnání metod stejného stupně (např. RK4 a Ralstonovy metody 4. stupně) jsme neshledali téměř žádné rozdíly.
...
Poté vytvoříme nové spinnery, jejichž úhlové rychlosti jsou zároveň částečně ovlivněny úhlovými zrychleními z $k_1$ a budeme odhadovat jejich úhlové rychlosti po uplynutí $a_{2, 1} \cdot \Delta t$: 
$${\Omega_1'}^{\mathbb{P}} (k) ^{\omega} = {\Omega_1}^{\mathbb{P}} (k) ^{\omega} + k_1 \cdot a_{2, 1} \cdot \Delta t$$
Takto vytvořené spinnery necháme zrychlovat po dobu $c_2 \cdot \Delta t$: 
$${\Omega_1''}^{\mathbb{P}} (k) ^{\omega} ={\Omega_1}^{\mathbb{P}} (k) ^{\omega} + {\Omega_1'}^{A} (k) \cdot c_{2} \cdot \Delta t$$
Nakonec získáme $k_2$ jako úhlové zrychlení těchto zrychlených (a pootočených) spinnerů: 
$$k_2 = {\Omega_1''}^{A}$$
Obecně můžeme tedy vyjádři $k_{n+1}$ takto: 
\begin{equation}
    \label{eq:sim_equations_formalized6}
    \begin{gathered}
        k_{n+1} = \Bigg( {\Omega_n}^{\mathbb{P}} (k) ^{\omega} + \Bigg( \sum_{i=1}^{n} ( k_i \cdot a_{n+1, i} \cdot \Delta t) \Bigg) \cdot c_{n+1} \cdot \Delta t \Bigg)^A
    \end{gathered}
\end{equation}
...
\begin{equation}
    \label{eq:sim_equations_formalized6}
    \begin{gathered}
        {\Omega_{n+1}}^{\mathbb{P}} (k) ^{\omega} = \Omega_{n}^{\mathbb{P}} (k) ^{\omega} + \Bigg(\sum_{i=1}^{s} ( b_i k_i)\Bigg) \cdot \Delta t
    \end{gathered}
\end{equation}

Analogicky funguje naprosto identický postup pro $\Phi$-stavy.

\subsection{Časová komplexita}

Pro dokončení simualce trvající $T$ a časovým intervalem $\tau$ je potřeba udělat $T/\tau$ iteračních kroků. V každém kroku počítáme interakce každého magnetu s každým - pokud máme $M$ spinnerů a každý z nich má $N$ ramen, získáváme $(NM)^2$ interagujících dvojic. K tomu musíme provádět více výpočtů, abychom zpřesnili výsledky. Použijeme-li RK metodu $K$. stupně a potřebujeme v každém kroku provést $U$ numerických integrací (pro nás je $U = 2$), získáváme finální časovou komplexitu:
