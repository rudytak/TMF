...
\begin{table}[!ht]
    ...
    \begin{tabularx}{\textwidth}{m{0.15\textwidth} m{0.1\textwidth} p{0.45\textwidth} p{4cm} }
        ...
        $\tau_F, \tau_{mag}$                 & $Nm$
                                             & Momenty sil působící na spinner vycházející ze silové a magnetické interakce & \cite{magnetic_torque,torque_def}, viz rov. \ref{eq:sim_equations2}                                                                  \\
    \end{tabularx}
\end{table}
...
\begin{table}[!ht]
    ...
    \begin{tabularx}{\textwidth}{p{0.15\textwidth} p{0.15\textwidth} p{0.2\textwidth} p{0.5\textwidth}}
        \textbf{Symbol}              & \textbf{Datový typ}                & \textbf{Iniciální hodnota} & \textbf{Význam}                                            \\
        \hline
        % $r$                          & \texttt{float64}                   & 0.0359                     & Poloměr                                                    \\
        % $S$                          & \texttt{v3}                        & žádná                      & Střed                                                      \\
        % $I$                          & \texttt{float64}                   & $4.80\cdot10^{-5}$         & Moment setrvačnosti                                        \\
        % $φ_0$                        & \texttt{float64}                   & žádná                      & Počáteční úhel                                             \\
        % $φ$                          & \texttt{float64}                   & žádná                      & Okamžitý úhel                                              \\
        % $ω_0$                        & \texttt{float64}                   & žádná                      & Počáteční úhlová rychlost                                  \\
        % $ω$                          & \texttt{float64}                   & žádná                      & Okamžitý úhlová rychlost                                   \\
        % \texttt{constant\_}$ω$       & \texttt{bool}                      & false                      & Určuje, zda má úhlová rychlost spinenru zůstat konstantní  \\
        % $α$                          & \texttt{float64}                   & 0.868                      & Konstantní koeficient tření                                \\
        % $β$                          & \texttt{float64}                   & 0.0                        & Lineární koeficient tření                                  \\
        % $γ$                          & \texttt{float64}                   & 0.00068                    & Kvadratický koeficient tření                               \\
        % $n$                          & \texttt{int}                       & 3                          & Počet ramen/magnetů                                        \\
        % $m_0$                        & \texttt{float64}                   & 0                          & Velikost mag. momentu                                      \\
        % \texttt{magnet\_orientation} &                                    &                            & Orientace magnetů                                          \\
        % $m(i)$                       & funkce \texttt{(int -> v3)}        & nelze měnit                & Funkce vracející orientaci magnetu podle jeho indexu       \\
        % $P(i)$                       & funkce \texttt{(int -> v3)}        & nelze měnit                & Funkce vracející pozici magnetu podle jeho indexu          \\
        % \texttt{copy ()}             & funkce \texttt{(empty -> spinner)} & nelze měnit                & Funkce vracející novou instanci spinneru se stejným stavem
    \end{tabularx}
\end{table}
