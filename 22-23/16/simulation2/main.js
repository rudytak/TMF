...
let par0 = {
  ...
  T_drive: 1 / 500, // [s] period of the driving oscillation
  tau: 1e-7, // [s] simulation time step
  runtime: 0.08, // [s] duration od the simulation
  ...
  saveToFile: false,
  ouputFile: "./out.csv",
  outputData: ["sim_t", "totD", "sum_acoustic_energy/sim_t"],
  save_period: 1e1,

  returnData: [
    "params.heater_temp",
    "sum_acoustic_energy/sim_t",
  ],
};
...
async function runSim(params) {
  ...
  var cols = params.outputData;
  var savePath = params.ouputFile;
  var actual_save_period = params.save_period;
  ...
  let heaters = [
    ...
    new TempArea(0.0, 0.15, 20 + absZ, 0.000015, "cooler"),
  ];

  // PREPARE ITERATION STEP
  function iter() {
    let p_glob = p_0 + Math.sin(omega_drive * sim_t) * p_amp;
    let p_prev = p_0 + Math.sin(omega_drive * (sim_t - tau)) * p_amp;
    for (let l of layers) {
    ...
    for (let lay of layers) {
      for (let h of heaters) {
        lay.accept_temp(h, totD, tau);
      }
      totD += lay.d;
    }

    let _power = (p_glob - p_0) * S * (totD - prevD);
    sum_acoustic_energy += _power;
    // sum_acoustic_energy += (p_prev - p_glob) * S * (totD - prevD)
    // console.log((p_glob - p_0) * S * (totD - prevD) / tau)
    outvals = cols.map((x) => eval(x));
  ...
  function addData(dat) {
    if (!params.saveToFile) return;

    bufferSize++;

    if (bufferSize % actual_save_period == 0) {
      buffer += dat.join(",") + "\n";
    }

    if (bufferSize >= maxBufferSize) {
      fs.appendFileSync(savePath, buffer, console.log);
  ...
  return new Promise((res, rej) => {
    if (params.saveToFile) {
      fs.writeFile(savePath, cols.join(",") + "\n", () => {
        for (var t = 0; t < run_time; t += tau) {
          let dat = iter();
          addData(dat);
        }
        // One last save of the leftovers
        fs.appendFileSync(savePath, buffer, console.log);

        console.log("DONE " + savePath);

        res(params.returnData.map((x) => eval(x)));
      });
    } else {
      for (var t = 0; t < run_time; t += tau) {
        iter();
...
async function main() {
  for (var t = 150; t <= 3000; t+=150) {
    let par = Object.assign({}, par0);
    par.heater_temp = t + absZ

    let res = await runSim(par);
    console.log(res);
  }
}
